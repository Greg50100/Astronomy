"""
Ce script utilise les bibliothèques `astronomy` et `paho.mqtt.client` pour publier des données astronomiques sur les planètes du système solaire via MQTT.

Fonctionnalités :
- Configure un client MQTT et se connecte à un broker MQTT.
- Obtient la date actuelle et définit un observateur à Cherbourg.
- Calcule diverses données astronomiques pour chaque planète (coordonnées équatoriales, élongation, illumination, constellation, altitude, azimut, heures de lever et de coucher, conjonctions, périhélie et aphélie).
- Publie les données calculées sur des topics MQTT spécifiques pour chaque planète.
...
Configuration MQTT :
- Adresse du broker : 192.168.x.xxx
- Port : 1883
...
Fonction principale :
- `publish_planet_data()`: Calcule et publie les données astronomiques pour chaque planète.

Boucle principale :
- Exécute `publish_planet_data()` toutes les 60 secondes.
"""
...
observer = astronomy.Observer(latitude=49.64, longitude=-1.62, height=7)
