...
def get_moon_information(location, time, timezone):
    ...
    with solar_system_ephemeris.set('de432s'):
        ...
        dot_product = np.dot(moon_geo.cartesian.xyz.value, sun_geo.cartesian.xyz.value)
        norm_moon = np.linalg.norm(moon_geo.cartesian.xyz.value)
        norm_sun = np.linalg.norm(sun_geo.cartesian.xyz.value)
        cos_phase_angle = dot_product / (norm_moon * norm_sun)
        cos_phase_angle = np.clip(cos_phase_angle, -1, 1)  # Ensure the value is within the valid range
