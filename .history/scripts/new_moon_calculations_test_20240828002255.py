...
def get_moon_information(location, time, timezone):
  ...
  with solar_system_ephemeris.set('de432s'):
    ...
    # Transform to geocentric frame for position vectors
    moon_geo = moon.transform_to(frame='geocentric')
    sun_geo = sun.transform_to(frame='geocentric')

    # Moon illumination (improved calculation)
    moon_geoloc = moon_geo.location
    sun_geoloc = sun_geo.location
    phase_angle = np.arccos((moon_geoloc.x - sun_geoloc.x) / moon.distance.to(u.au))
